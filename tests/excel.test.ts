...
describe("Excel 浏览器解析与周报合成", () => {
  ...
  it("读取基本字段、固定节点并标记无法识别日期", async () => {
    ...
    ws.addRow(["项目编号","项目名称","项目类别","合同类型","合同金额","上家单位","市场负责人","项目经理","项目组","当前项目状态","备注","到货计划","到货实际","验收计划","验收实际"]);
    ws.addRow(["X-001","虚构测试项目","示例","服务","100万元","示例单位","市场甲","经理甲、经理乙","项目A组","实施中","测试备注","日期不明","","2026-09-01",""]);
    const result = await parseProgressSheet(await bytes(wb), "test.xlsx");
    expect(result.projects).toHaveLength(1); expect(result.projects[0].marketOwner).toBe("市场甲"); expect(result.projects[0].team).toBe("A");
