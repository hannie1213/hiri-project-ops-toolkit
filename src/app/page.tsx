...
import { Download, FilePlus2, FileSpreadsheet, Search } from "lucide-react";
...
export default function DashboardPage() {
  ...
  return <div className="space-y-5">
    {showHelp && <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 p-4"><Card className="max-w-lg p-6"><h2 className="text-lg font-bold">首次使用</h2><p className="mt-3 text-sm leading-6 text-slate-600">请先进入“导入 Excel/CSV”，选择包含“所有项目进度计划情况”工作表的文件。文件只在浏览器本地解析，不会上传服务器。</p><p className="mt-2 text-sm font-medium text-blue-700">本工具数据保存在当前浏览器，不会自动同步到其他电脑。</p><Button className="mt-5" variant="primary" onClick={() => { saveSettings({firstVisitSeen:true}); setShowHelp(false); }}>我知道了</Button></Card></div>}
    <div><h1 className="text-xl font-bold text-slate-900">首页监管台</h1><p className="mt-1 text-sm text-slate-500">共 {rows.length} 个项目 · 数据仅保存在当前浏览器</p></div>
    ...
    <div className="flex flex-wrap gap-2"><Link href="/import"><Button variant="primary"><FileSpreadsheet className="h-4 w-4"/>导入 Excel/CSV</Button></Link><Link href="/projects/new"><Button variant="secondary"><FilePlus2 className="h-4 w-4"/>人工录入</Button></Link><Button variant="secondary" onClick={() => exportRows("xlsx")}><Download className="h-4 w-4"/>导出提醒清单</Button><Link href="/weekly"><Button variant="secondary">周报合成</Button></Link><Link href="/confirm"><Button variant="secondary">项目状态确认</Button></Link></div>
